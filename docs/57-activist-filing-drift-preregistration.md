...
### 判定: ❌ FAIL
- 最良 act_accum_reg_h40 で SR(ann)+0.08・**DSR=0.08**。主セル act_new_reg_h40 は SR−0.10。
  副次基準 (i)(ii)(iii) 未達（容量も¥万単位）＝機械的 FAIL。K=4・tradable イベント 6,495件
  （sec_code 復元は edinetCode 対応表で機能・名簿新規277・重要提案381・積増し689）。
- レポート: `data/reports/activist_filing_drift.html`／スクリプト: `examples/research_activist_filing.py`
  ／再利用資産: `data/processed/edinet_code_map.parquet`（EDINETコード→証券コード対応・汎用）。

### 機構診断（豊富な収穫）
- **filer 異質性は日本論文どおり実在**: イベントスタディ（対ヘッジ超過累積）で
  名簿filer新規 **+386bps/5日**・重要提案 +305bps/5日・全filer **−73bps/40日**（無）。
  H1 の統制超え＝ΔSR +0.95・差分系列 SR +0.52。Gillan et al. の異質性は再現。
- **だが「ドリフト」でなく「ジャンプ」**: 超過は最初の5日（実質1-2日）でほぼ完結し、
  以後60日までフラット（+386→+310→+294→+329→+329）。T+1 執行後に取れる成分は薄く、
  40日保有は残り35日の特異ボラを抱えるだけ＝ポートフォリオ SR に変換されない。
- **標本内で急減衰**: 最良セル年次 SR 2022 +0.97 → 2023 +0.86 → **2024 +0.01 → 2025 −1.23**。
  アクティビズム・ブーム（2024-25 に報告件数最多）と同時に crowding で即時織り込み化＝
  IS+0.47→OOS−0.85。
- F1 診断: ρ(value L/S 月次)=+0.14（軽度）。H-16 診断: 対象銘柄の momentum 分位中央値 0.67
  （急騰後に filer が入る＝軽度のモメンタム傾き）。
- コスト: 主セルは 0bps でも SR+0.19＝コスト以前に「残余ドリフトが細い」が根本。

### 失敗型分類と収穫
- 分類: **F2/F6（標本内減衰・OOS負）＋F8 亜型（イベント平均効果は実在するが
  ジャンプ型＝T+1 後の取れる成分が細く、ポートフォリオ表現で溶解）**。
- 収穫①: **H-17 として heuristic 化**＝「日本の公開開示イベントの織り込みは 2024+ で高速化
  （数日で完結するジャンプ型）。イベント仮説は『T+1 以降に残る成分が構造的に大きい理由』を
  事前に説明できなければ優先度を下げる」。
- 収穫②: filer 異質性の再現＝**開示の「読み方」の情報自体は本物**（名簿・重要提案フラグは
  リスク管理・除外スクリーン用途の部品として有効）。
- 収穫③: edinetCode→secCode 対応表は汎用再利用可（EDINET 系イベント研究全般の配管）。
- 再評価条件: なし（打ち止め）。日中執行（開示当日の場中約定）が可能になれば別仮説として再登録可。
