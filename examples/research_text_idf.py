r"""候補①テキストα Stage 1.5：脱飽和（変動の大きい節＋trailing-IDF）。

Stage1（全MD&A・char-n-gram 素のコサイン）は前年比類似度が中央値0.965＝定型文に飽和しFAIL。
本版は (1) 節を **事業等のリスク／対処すべき課題** に絞り、(2) **trailing-IDF**（各後者年で過去
有報コーパスに fit＝PIT）で定型語を減衰し、「記述変化」を立てて再検定する。判定は同一 scope
`disclosure_text_change` に積む＝K増でデフレート（同一仮説の反復＝バーを上げる＝anti-p-hacking）。

実行: $env:J_QUANTS_MIN_INTERVAL="0.7"; .venv\Scripts\python.exe examples\research_text_idf.py
"""
...
TOP_N, QS = 500, [0.1, 0.2, 0.3]
IDX_CACHE = Path("data/processed/edinet_annual_index.parquet")
NARROW_CACHE = Path("data/processed/disclosure_text_narrow.parquet")
...
def load_narrow(superset) -> pd.DataFrame:
    """有報の **事業リスク/対処課題** のみ抽出（キャッシュ・冪等再開）。"""
    idx = pd.read_parquet(IDX_CACHE) if IDX_CACHE.exists() else dtx.annual_report_index()
    ...
    cached = (pd.read_parquet(NARROW_CACHE) if NARROW_CACHE.exists()
    ...
    for i, (_, r) in enumerate(todo.iterrows()):
        ...
        txt = dtx.extract_narrative(zp, dtx.NARROW_KEYWORDS) if zp.exists() else ""
        ...
        if (i + 1) % 1500 == 0:
            ...
            new = []
            cached.to_parquet(NARROW_CACHE)
            print(f"  抽出 {i + 1}/{len(todo)} …")
    if new:
        cached = pd.concat([cached, pd.DataFrame(new)], ignore_index=True)
        cached.to_parquet(NARROW_CACHE)
    t = cached[cached["Code"].isin(superset)].copy()
    return t[t["text"].astype(str).str.len() > 100][["Code", "submitDateTime", "text"]]
...
def main() -> int:
    if not get_env("J_QUANTS_API_KEY"):
        ...
        return 1
    print(f"=== ①テキストα Stage1.5（脱飽和: 節絞り＋trailing-IDF） {START}〜{END} ===")
    ...
    texts = load_narrow(set(superset))
    print(f"事業リスク/対処課題 保有 {len(texts)} 件 / {texts['Code'].nunique()} 銘柄")
    sim = dtx.yoy_text_similarity_idf(texts).dropna(subset=["text_sim"])
    print(f"trailing-IDF 前年比類似度 {len(sim)} 件（中央値 {sim['text_sim'].median():.3f}・"
          f"標準偏差 {sim['text_sim'].std():.3f}）← Stage1は中央値0.965で飽和")
    ...
    val, mom, sz = prep(vqs["book_to_market"]), prep(vqs["momentum"]), prep(vqs["size"])

    strats = [CrossSectionalStrategy(sig, q, name=f"text_idf(q={q})") for q in QS]
    with default_registry() as reg:
        v = judge_grid(
            strats, view, scope="disclosure_text_change",
            hypothesis="有報の事業リスク/対処課題のtrailing-IDF前年比類似度が低い（記述を書き換えた）"
                       "銘柄は将来アンダーパフォーム（Lazy Prices・脱飽和版）",
            economic_rationale="定型語を減衰し記述変化を抽出。経営者の前向き叙述の書き換えは数値に出ない"
                               "soft information のアンダーリアクション。価格/ファンダと独立。",
            registry=reg, costs_bps=15.0, adv=adv, participation=0.1, execution_lag=1)
    print("\n" + v.report_md)
    print("HTML:", write_html(v, f"data/reports/{v.scope}_idf.html"))

    print("\n--- 独立性（text_idf と既知factorの月平均XS相関）---")
    ...
    print(f"  小型 SR={_ls_sharpe(sig, fwd, small):+.2f} / 大型 SR={_ls_sharpe(sig, fwd, large):+.2f}")
